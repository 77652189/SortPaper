...
当前上下文级代理评测显示：在默认 lexical 召回和锚点保护开启时，加入 paper-local + neighbor context 后，exact `context_chunk_hit@10` 从 0.5000 提升到 0.5667，`context_nearby_hit@10` 从 0.5667 提升到 0.6000。因此后续重点应是继续提升初始 top evidence 和论文内 evidence scoring，而不是单纯增加邻近 chunk 数量。
