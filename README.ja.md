...
MinerU external extraction smoke test は `MINERU_API_KEY` を使います。`MINERU_MODEL_VERSION` の既定値は `vlm` です。
...
手動検索と Agent 検索では、enhanced chunk recall がデフォルトで有効です。手動検索はデフォルトで 10 件を表示するため、評価済みの top10 evidence をすぐ確認できます。この indexed lexical backfill は `search_text` と低頻度 query term を使って evidence 候補を補い、毎回ライブラリ全体を走査せず、元の上位検索アンカーも保護します。rerank なしの 60-case top10 評価では `chunk_hit@10` が `0.4000` から `0.6000` に、`nearby_chunk_hit@10` が `0.4000` から `0.6333` に改善し、p50 latency は約 `561ms` から `745ms` になりました。UI デフォルトの qwen3-rerank 経路では `chunk_hit@10` が `0.6667`、`nearby_chunk_hit@10` が `0.7000`、p50 latency が約 `1621ms` です。広すぎる query や遅延が気になる場合は UI で無効化できます。
