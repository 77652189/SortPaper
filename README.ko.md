...
수동 검색과 Agent 검색 UI는 검색 route, 정규화 query, entity fields, context-localization query, hit 수, latency, 결과별 matched route/query를 표시합니다. 이 정보는 recall debugging을 위한 것이며 저장된 payload를 변경하지 않습니다.
...
- Agent paper-local context는 정규화 query에 products, organisms, genes, enzymes, metrics, aliases, 앞의 2개 variants를 더한 짧은 evidence-localization query를 사용합니다. `table` 또는 `text` evidence preference는 이미 hit된 논문 안에서 작은 ranking bonus만 주며, 다른 content type을 필터링하지 않습니다.
...
Agent context evaluation도 production path에 가깝게 rerank, query rewrite, multi-query recall을 켜서 실행할 수 있습니다:
